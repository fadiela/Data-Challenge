...
if not has_any_selection:
    ...
    st.markdown(
        ...
        f'<div class="summary-label">Hasil Mencakup {len(result_df)} Baris</div>'
        f'<div class="summary-value">Kombinasi filter menghasilkan lebih dari satu baris, jadi Norm Value &amp; Base N tunggal tidak ditampilkan. Lihat tabel detail di bawah untuk rincian tiap kombinasi.</div>'
        f'</div>',
        unsafe_allow_html=True
    )
